...
def query_matcha(
    ...
    amount = float(amount)
    coin1_name = coin1[0]
    coin1_address = coin1[1]
    coin2_name = coin2[0]
    coin2_address = coin2[1]

    # Get page to scrape
    url = f"https://matcha.xyz/markets/1/{coin2_address}/{coin1_address}"
    driver.get(url)

    input1_xpath = "//*[@id='trading-page-container']/div[2]/div/div/aside/div[1]/div/div/div[4]/div[2]/input"
    input1 = WebDriverWait(driver, request_wait_time).until(ec.presence_of_element_located(
        (By.XPATH, input1_xpath)))
    ...
    input2_xpath = "//*[@id='trading-page-container']/div[2]/div/div/aside/div[1]/div/div/div[6]/div[3]/div[2]/input"
    input2 = WebDriverWait(driver, request_wait_time).until(ec.presence_of_element_located(
        (By.XPATH, input2_xpath)))
